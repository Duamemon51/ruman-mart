  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#f5f7fb] text-slate-800">
      ...
      <main className="flex-1">
        <section className="relative isolate overflow-hidden bg-[#031a3b]">
          ...
          <div className="relative mx-auto flex min-h-[240px] max-w-[1400px] items-center px-5 py-7 sm:min-h-[280px] md:min-h-[340px] md:px-8 lg:min-h-[380px]">
            <div className="max-w-2xl text-white">
              ...
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#19d5f2] sm:text-sm">Fresh in store</p>
              <h1 className="mt-2 text-3xl font-bold leading-[1.05] sm:text-5xl">
                New <span className="text-[#19d5f2]">Arrivals</span>
              </h1>
        ...
        <section className="mx-auto max-w-[1400px] px-4 py-10 md:px-8 sm:py-14">
          <div className="mb-6 flex items-end justify-between gap-4">
            <div>
              <h2 className="text-2xl font-bold text-[#0b1d45] sm:text-3xl">Just Added</h2>
              <p className="mt-1 text-sm text-slate-500">Explore our latest products.</p>
            </div>
            <Link href="/categories" className="text-sm font-semibold text-[#0b75a5] hover:text-[#064d70]">View Categories</Link>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            {products.map((product) => (
              <article
                key={product.name}
                className="group flex flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition-all hover:-translate-y-0.5 hover:border-[#19c9ee] hover:shadow-md"
              >
                <div className="relative flex h-36 items-center justify-center bg-white p-3">
                  <WishlistButton product={product} />
                  <span className="absolute right-2 top-2 rounded-full bg-[#0b75a5] px-2 py-0.5 text-[10px] font-bold text-white">New</span>
                ...
                <div className="flex flex-1 flex-col gap-1 border-t border-slate-100 p-3">
                  <p className="text-[11px] font-semibold text-[#0b75a5]">{product.brand}</p>
                  <h3 className="truncate text-sm font-bold text-[#0b1d45]">{product.name}</h3>
                  <div className="flex items-center gap-1"><StarRating rating={product.rating} /><span className="text-[10px] text-slate-400">({product.reviews.toLocaleString()})</span></div>
                  <div className="mt-auto flex items-baseline gap-1.5 pt-2"><span className="text-sm font-bold text-[#0b1d45]">{formatPrice(product.price)}</span><span className="text-[10px] text-slate-400 line-through">{formatPrice(product.originalPrice)}</span></div>
                  <Link href={`/categories/electronics/${productSlug(product.name)}`} className="mt-2 inline-flex items-center justify-center gap-1 rounded-lg bg-[#0b1d45] py-2 text-xs font-semibold text-white"><ShoppingCart size={13} aria-hidden="true" />View Details</Link>
                </div>
              </article>
            ))}
          </div>
